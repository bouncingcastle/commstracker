import { Record } from '@servicenow/sdk/core'

Record({
    $id: Now.ID['469b7c6283937210a0365dc6feaad354'],
    table: 'sys_ui_element',
    data: {
        element: '.begin_split',
        position: 0,
        sys_ui_section: 'ce9bb0ae83537210a0365dc6feaad374',
        type: '.begin_split',
    },
})
Record({
    $id: Now.ID['029b7c6283937210a0365dc6feaad355'],
    table: 'sys_ui_element',
    data: {
        element: 'commission_plan',
        position: 1,
        sys_ui_section: 'ce9bb0ae83537210a0365dc6feaad374',
    },
})
Record({
    $id: Now.ID['ca9b7c6283937210a0365dc6feaad355'],
    table: 'sys_ui_element',
    data: {
        element: '.split',
        position: 2,
        sys_ui_section: 'ce9bb0ae83537210a0365dc6feaad374',
        type: '.split',
    },
})
Record({
    $id: Now.ID['869b7c6283937210a0365dc6feaad356'],
    table: 'sys_ui_element',
    data: {
        element: 'is_active',
        position: 3,
        sys_ui_section: 'ce9bb0ae83537210a0365dc6feaad374',
    },
})
Record({
    $id: Now.ID['429b7c6283937210a0365dc6feaad357'],
    table: 'sys_ui_element',
    data: {
        element: '.end_split',
        position: 4,
        sys_ui_section: 'ce9bb0ae83537210a0365dc6feaad374',
        type: '.end_split',
    },
})
Record({
    $id: Now.ID['0e9b7c6283937210a0365dc6feaad357'],
    table: 'sys_ui_element',
    data: {
        element: 'tier_name',
        position: 5,
        sys_ui_section: 'ce9bb0ae83537210a0365dc6feaad374',
    },
})
Record({
    $id: Now.ID['c69b7c6283937210a0365dc6feaad358'],
    table: 'sys_ui_element',
    data: {
        element: '.begin_split',
        position: 6,
        sys_ui_section: 'ce9bb0ae83537210a0365dc6feaad374',
        type: '.begin_split',
    },
})
Record({
    $id: Now.ID['829b7c6283937210a0365dc6feaad359'],
    table: 'sys_ui_element',
    data: {
        element: 'attainment_floor_percent',
        position: 7,
        sys_ui_section: 'ce9bb0ae83537210a0365dc6feaad374',
    },
})
Record({
    $id: Now.ID['4e9b7c6283937210a0365dc6feaad359'],
    table: 'sys_ui_element',
    data: {
        element: 'commission_rate_percent',
        position: 8,
        sys_ui_section: 'ce9bb0ae83537210a0365dc6feaad374',
    },
})
Record({
    $id: Now.ID['0a9b7c6283937210a0365dc6feaad35a'],
    table: 'sys_ui_element',
    data: {
        element: '.split',
        position: 9,
        sys_ui_section: 'ce9bb0ae83537210a0365dc6feaad374',
        type: '.split',
    },
})
Record({
    $id: Now.ID['c29b7c6283937210a0365dc6feaad35b'],
    table: 'sys_ui_element',
    data: {
        element: 'sort_order',
        position: 10,
        sys_ui_section: 'ce9bb0ae83537210a0365dc6feaad374',
    },
})
Record({
    $id: Now.ID['8e9b7c6283937210a0365dc6feaad35b'],
    table: 'sys_ui_element',
    data: {
        element: '.end_split',
        position: 11,
        sys_ui_section: 'ce9bb0ae83537210a0365dc6feaad374',
        type: '.end_split',
    },
})
Record({
    $id: Now.ID['4a9b7c6283937210a0365dc6feaad35c'],
    table: 'sys_ui_element',
    data: {
        element: 'description',
        position: 12,
        sys_ui_section: 'ce9bb0ae83537210a0365dc6feaad374',
    },
})
